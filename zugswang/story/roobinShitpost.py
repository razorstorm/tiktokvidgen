...
def setup_scenes_from_file(filename: str) -> None:
    ...
    for scene_data in scenes_dict["scenes"]:
        add_scene(
            ...
            narration=scene_data.get("narration", None),
...
if __name__ == '__main__':
    ...
    image_clip = moviepy.editor.ImageClip(bg_image_filepath)
    bg_width, bg_height = image_clip.size
    image_clip = (
        image_clip
        # .fl_image(lambda image: np.array(Image.fromarray(image).convert('RGB')))  # sometimes the image is missing a channel (?)
        .fx(vfx.resize, width=width, height=height)
        .resize(width=width, height=height)
    )
    # image_clip.set_width(width)
    # image_clip.set_height(height)
    background_audio = moviepy.editor.AudioClip(lambda t: 0, duration=1)
    
    background_audio = moviepy.editor.AudioFileClip(os.path.join("data", "audio", "bg_audio.mp3"))
    ...
    print("duration", background_audio.duration)
